...
YOLOP_lane_robot_no_ll_seg = [  # 去除了车道线分割层
    [24, 33, 34],  # Det_out_idx, Da_Segout_idx, LL_edt_idx  去除了车道线分割层
    [-1, Focus, [3, 32, 3]],  # 0            1 12 320x320(输出尺寸）
    [-1, Conv, [32, 64, 3, 2]],  # 1 conv    1 64 160 160
    [-1, BottleneckCSP, [64, 64, 1]],  # 2   1 64 160 160
    [-1, Conv, [64, 128, 3, 2]],  # 3 conv   1 128 80 80
...
class YOLOP_Lane_net(nn.Module):
    def __init__(self, block_cfg, **kwargs):
        ...
        # 检查是否存在新的车道线解析头索引
        self.ll_parsing_index = block_cfg[0][3] if len(block_cfg[0]) > 3 else block_cfg[0][2]

        self.ll_seg_index = block_cfg[0][2] if len(block_cfg[0]) > 3 else -1
