...
def main():
    ...
    print("加载模型到设备")
    model = get_net(cfg).to(device)  # 根据配置获取模型并将其移动到指定设备
    ...
    Da_Seg_Head_para_idx = [str(i) for i in range(25, 34)]  # 车道线
    ...
    model.nc = 1  # 类别数量，这里设为1可能表示是单任务或特定处理
    ...
    train_dataset = eval('dataset.' + cfg.DATASET.DATASET)(
        cfg=cfg,
        is_train=True,  # 训练模式
        inputsize=cfg.MODEL.IMAGE_SIZE,  # 输入图像大小
